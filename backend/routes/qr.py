...
def generate_qr(current_user):
    ...
    if not profile:
        ...
        if not user_row or not user_row.get("blood_group"):
            return jsonify({
                "success": False,
...
def view_qr(token):
    ...
    user_info = query_db(
        "SELECT id, name, blood_group, phone, emergency_contact, emergency_phone, allergies, medications, conditions FROM users WHERE id = ?",
        (user_id,), one=True
    )
    ...
    now_iso = datetime.now(timezone.utc).isoformat()
    execute_db(
        "INSERT INTO access_logs (id, user_id, access_type, accessed_at) VALUES (?, ?, 'QR_SCAN', ?)",
        (log_id, user_id, now_iso)
    )

    emergency_data = {
        "patient_name": user_info.get("name") if user_info else "Patient",
        "blood_group": (profile.get("blood_group") if profile else user_info.get("blood_group")) or "Not specified",
        "emergency_phone": (profile.get("phone") if profile else user_info.get("emergency_phone") or user_info.get("phone")) or "",
        "emergency_contact": (profile.get("emergency_contact") if profile else user_info.get("emergency_contact")) or "",
        "allergies": (profile.get("allergies") if profile else user_info.get("allergies")) or "None reported",
        "medications": (profile.get("medications") if profile else user_info.get("medications")) or "None reported",
        "conditions": (profile.get("conditions") if profile else user_info.get("conditions")) or "None reported"
    }
